...
pub fn finish(stream: &mut TcpStream, task_id: String, result: TaskReturn) -> Result<Response, Error> {
    let req = Request {
        kind: Some(ReqKind::FinishTask(message::FinishTask { task_id, result: Some(result) })),
    };
    message::write(stream, &req)?;
    let response = message::read_response(stream)?;
    Ok(response)
...
pub fn labeled_invoke(stream: &mut TcpStream, labeled_invoke: message::LabeledInvoke) -> Result<(), Error> {
    let req = Request {
