...
use rouille::{Request, Response, input::post::BufferedFile};
use labeled::buckle;
use snapfaas::{fs::{self, FS}, blobstore::Blobstore, sched::{self, message::LabeledInvoke}};
...
fn prepare_payload(request: &Request, blobstore: Arc<Mutex<Blobstore>>) -> Result<(String, buckle::Buckle), Response> {
    // Parse input into a 3-tuple. support two content types:
    // * multipart/form
    // * application/json (passthrough)
    let (maybe_file, payload, label) = {
        let content_type = request.header("content-type").ok_or(
        ...
        if content_type == "multipart/form-data" {
            ...
            let _ = request.data().unwrap().read_to_string(&mut payload).map_err(|e|
                Response::json(&serde_json::json!({
                    "error": e.to_string()
                })).with_status_code(400))?;
            (None, payload, buckle::Buckle::public())
        } else {
            return Err(Response::json(&serde_json::json!({"error": "Unsupported content-type"}))
                .with_status_code(400));
        }
    };
    ...
    if let Some(f) = maybe_file {
        ...
        let name = blobstore.lock().unwrap().save(newblob).map_err(|e|
            Response::json(&serde_json::json!({"error": e.to_string()})).with_status_code(500)
        )?.name;
...
fn prepare_labeled_invoke(gate_path: String, payload: String, fs: &FS<&lmdb::Environment>) -> Result<sched::message::LabeledInvoke, Response> {
...
fn wait_for_completion(invoke: LabeledInvoke, sched_conn: &mut TcpStream) -> Result<Response, Response> {
